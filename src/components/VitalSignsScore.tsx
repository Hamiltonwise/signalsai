...
import { TrendingUp, TrendingDown, Activity, Sparkles, Lightbulb } from 'lucide-react';
import { getLastFullMonth } from '../utils/dateUtils';
...
export const VitalSignsScore: React.FC<VitalSignsScoreProps> = ({
  ...
  const getStatusBadge = () => {
    if (score >= 85) return { text: '🚀 Improving Fast', color: 'bg-green-100 text-green-800' };
    if (score >= 70) return { text: '📈 Steady Growth', color: 'bg-blue-100 text-blue-800' };
    if (score >= 55) return { text: '✅ Holding Strong', color: 'bg-teal-100 text-teal-800' };
  ...
  const getTrendMessage = () => {
    ...
    if (trend === 'up' && monthlyChange > 0) {
      ...
      return `${Math.abs(monthlyChange)}% change vs ${lastMonth.label} — steady performance`;
    }
  };
  ...
  return (
    <div className={`bg-white rounded-2xl shadow-xl border border-gray-100 p-8 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
      <div className="flex items-start justify-between mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-bold text-gray-900 tracking-tight">Practice Vital Signs</h2>
            <p className="text-gray-600">Real-time performance across all metrics</p>
          </div>
        </div>
        
        <div className="text-right">
          <span className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium ${statusBadge.color}`}>
            {statusBadge.text}
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-center">
          ...
          <div className="relative inline-flex items-center justify-center">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-center">
                ...
                <div className="text-sm text-gray-500 font-medium">Vital Signs</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-gray-700">Performance insights updated daily</span>
            </div>
            <div className="text-xs text-gray-500">
              Based on website traffic, search visibility, and local presence data
            </div>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl p-6">
          <h4 className="font-semibold text-gray-900 mb-4">This Month's Highlights</h4>
