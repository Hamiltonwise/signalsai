...
import { X, Upload, FileText, Download, CheckCircle, AlertCircle, Info } from 'lucide-react';
...
export const PMSUploadModal: React.FC<PMSUploadModalProps> = ({
  ...
  const parseCSV = (file: File) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        console.log('PMSUploadModal: Raw CSV text loaded (first 500 chars):', text.substring(0, 500) + '...'); // Log first 500 characters
        
        const lines = text.split('\n').filter(line => line.trim() && !line.startsWith('#'));
        console.log('PMSUploadModal: CSV lines after filtering:', lines.length, 'lines');
        if (lines.length > 0) {
        ...
        const parseAndConvertDate = (dateStr: string): string | null => {
          ...
          for (let i = 0; i < formats.length; i++) {
            ...
            if (match) {
              ...
              if (i === 0) { // MM/DD/YY or MM-DD-YY
                month = match[1].padStart(2, '0');
                day = match[2].padStart(2, '0');
                year = parseInt(match[3]);
                // Convert 2-digit year to 4-digit (assume 20xx for years 00-99)
                year = year < 50 ? 2000 + year : 1900 + year;
              } else if (i === 1) { // MM/DD/YYYY or MM-DD-YYYY
                month = match[1].padStart(2, '0');
                day = match[2].padStart(2, '0');
                year = parseInt(match[3]);
              } else if (i === 2) { // YYYY-MM-DD (already correct)
              ...
              console.log(`PMSUploadModal: Converted "${cleanDate}" to "${convertedDate}"`);
        ...
        const createFlexibleHeaderMap = (headers: string[]) => {
          ...
          headers.forEach(header => {
            ...
            else if (cleanHeader === 'referral_type' || cleanHeader === 'referraltype' || cleanHeader === 'referral type') {
              headerMap[header] = 'referral_type';
            }
            else if (cleanHeader === 'referral_source' || cleanHeader === 'referralsource' || cleanHeader === 'referral source') {
              headerMap[header] = 'referral_source';
            }
            else if (cleanHeader === 'production_amount' || cleanHeader === 'productionamount' || cleanHeader === 'production amount' || cleanHeader === 'production') {
              headerMap[header] = 'production_amount';
            }
            else if (cleanHeader === 'appointment_type' || cleanHeader === 'appointmenttype' || cleanHeader === 'appointment type') headerMap[header] = 'appointment_type';
            else if (cleanHeader === 'treatment_category' || cleanHeader === 'treatmentcategory' || cleanHeader === 'treatment category') headerMap[header] = 'treatment_category';
            else if (cleanHeader === 'notes' || cleanHeader === 'note') headerMap[header] = 'notes';
          });
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          const values = parseCSVLine(lines[i]).map(v => v.trim().replace(/^"|"$/g, '')); // Remove surrounding quotes
          const record: any = {};
          ...
          if (!convertedDate) {
            errors.push(`Row ${i + 1}: Invalid date format "${record.date}". Supported formats: MM/DD/YY, MM-DD-YY, MM/DD/YYYY, MM-DD-YYYY, YYYY-MM-DD`);
            continue;
          }
          
          const referralType = record.referral_type;
          const productionAmount = parseFloat(record.production_amount as any) || 0;
          ...
          if (trimmedReferralType.length > 100) {
            errors.push(`Row ${i + 1}: Referral_Type too long (max 100 characters)`);
            continue;
          }
          
          const normalizedReferralType = normalizeReferralType(trimmedReferralType);
          ...
          console.log(`PMSUploadModal: Processed record for row ${i + 1}:`, processedRecord);
  ...
  const handleUpload = async () => {
    ...
    try { // The csvData is already in the correct format for the backend
            ...
            <div className="space-y-6">
              <div className="text-center">
                ...
                <p className="text-gray-600">
                  Upload your monthly referral data to track practice growth and patient acquisition trends.
                </p>
              </div>
                  ...
                  <div>
                    ...
                    <div className="text-sm text-blue-800 space-y-1">
                      ...
                      <ul className="list-disc list-inside ml-4 space-y-1">
                        <li><code>Date</code> - Formats: MM/DD/YY, MM-DD-YY, MM/DD/YYYY, or YYYY-MM-DD (e.g., 01/15/24 or 2024-01-15)</li>
                        <li><code>Referral_Type</code> - Any text describing the referral source (e.g., 'Self-Referral', 'Dr. Smith', 'Insurance', etc.)</li>
                        <li><code>Production_Amount</code> - Numeric value (e.g., 350.00)</li>
                      </ul>
                      <p><strong>Optional columns:</strong></p>
                      <ul className="list-disc list-inside ml-4 space-y-1">
                        <li><code>Referral_Source</code>, <code>Appointment_Type</code>, <code>Treatment_Category</code>, <code>Notes</code></li>
                      </ul>
                    </div>
                  </div>
                </div>
              ...
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center hover:border-blue-400 transition-colors">
                ...
                <p className="text-gray-600 mb-4">
                  {file ? 'File selected. Click below to continue.' : 'Select a CSV file to upload your PMS data'}
                </p>
                <button
          ...
          {step === 'preview' && (
            <div className="space-y-6">
              <div className="text-center">
                ...
                <p className="text-gray-600">
                  Review your data before uploading. Found {csvData.length} records.
                </p>
              </div>
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50 sticky top-0">
                      <tr>
                        ...
                        <th className="px-4 py-2 text-left font-medium text-gray-900">Referral Type</th>
                        <th className="px-4 py-2 text-left font-medium text-gray-900">Referral Source</th>
                        <th className="px-4 py-2 text-left font-medium text-gray-900">Production</th>
                        <th className="px-4 py-2 text-left font-medium text-gray-900">Appt. Type</th>
                        <th className="px-4 py-2 text-left font-medium text-gray-900">Treatment</th>
                    ...
                    <tbody>
                      {csvData.map((record, index) => (
                        <tr key={index} className="border-t border-gray-200">
                          ...
                          <td className="px-4 py-2">{record.referral_source || '-'}</td>
                          <td className="px-4 py-2">
                            {record.production_amount ? `$${record.production_amount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : '-'}
                          </td>
                          <td className="px-4 py-2">{record.appointment_type || '-'}</td>
                          <td className="px-4 py-2">{record.treatment_category || '-'}</td>
            ...
            <div className="text-center space-y-6">
                ...
                {uploadResult && (
                  <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                    <div className="text-sm text-green-800">
                      <p><strong>Records processed:</strong> {uploadResult.data?.recordsProcessed || 0}</p>
                      <p><strong>Records stored:</strong> {uploadResult.data?.recordsStored || 0}</p>
                    </div>
                  </div>
                )}
              </div>
              <div className="space-y-4">
                <div className="flex items-center justify-center gap-3">
                  ...
                  <span className="text-green-800 font-medium">Data successfully integrated into dashboard</span>
                </div>
                <button
                  onClick={() => {
                    onSuccess?.();
