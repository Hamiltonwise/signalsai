...
import { Target, ArrowRight, CheckCircle, Clock, AlertTriangle } from 'lucide-react';
  ...
  const getPriorityAction = () => {
    ...
    if (connectionStatus.gsc && gscData?.averagePosition > 5) {
      return {
        ...
        description: `Average position is ${gscData.averagePosition.toFixed(1)} - optimize for better visibility`,
        impact: 'High Impact',
    ...
    if (connectionStatus.gbp && gbpData?.averageRating < 4.5) {
      return {
        ...
        description: `Current rating is ${gbpData.averageRating.toFixed(1)}★ - focus on review improvement`,
        impact: 'High Impact',
    ...
    if (connectionStatus.ga4 && ga4Data?.engagementRate < 50) {
      return {
        ...
        description: `Engagement rate is ${ga4Data.engagementRate.toFixed(1)}% - optimize content`,
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
      ...
      <div className="space-y-4">
        <div className={`bg-gradient-to-r ${priorityAction.color} rounded-lg p-4`}>
          <div className="flex items-center justify-between mb-3">
            <h4 className={`font-semibold ${priorityAction.textColor}`}>Priority Action</h4>
            <span className={`text-xs ${priorityAction.badgeColor} text-white px-2 py-1 rounded-full`}>
          ...
          <div className="flex items-center justify-between">
            <span className={`text-xs ${priorityAction.textColor.replace('800', '600')}`}>
              {priorityAction.estimate}
            </span>
            <button className={`${priorityAction.badgeColor} text-white px-3 py-1 rounded-lg text-sm hover:opacity-90 flex items-center space-x-1 transition-all`}>
              <span>Start</span>
        ...
        <div className="space-y-2">
          ...
          {upcomingActions.map((action, index) => (
            <div key={index} className="flex items-center space-x-3 p-2 rounded-lg hover:bg-gray-50 transition-colors">
              ...
              <span className={`text-sm flex-1 ${action.status === 'completed' ? 'line-through text-gray-500' : 'text-gray-600'}`}>
                {action.title}
              </span>
              <span className={`text-xs ${action.status === 'completed' ? 'text-green-600' : 'text-gray-400'}`}>
                {action.date}
              </span>
            </div>
          ))}
