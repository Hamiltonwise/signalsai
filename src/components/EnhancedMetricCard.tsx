...
export const EnhancedMetricCard: React.FC<EnhancedMetricCardProps> = ({
  ...
  const getDataSourceBadge = () => {
    ...
    return (
      <span className={`px-2 py-1 text-xs rounded-full font-medium ${badgeColors[dataSource]}`}>
        {dataSource}
      </span>
    );
  };
        ...
        <div className="w-full bg-gray-200 rounded-full h-2">
          <div 
            className={`h-2 rounded-full transition-all duration-1000 ${
              ...
              trend === 'down' ? 'bg-gradient-to-r from-amber-400 to-amber-600' :
              'bg-gradient-to-r from-gray-400 to-gray-600'
            }`}
            style={{ 
              width: `${Math.min(Math.abs(parseFloat(change.replace(/[+\-%]/g, ''))), 100)}%` 
            }}
          />
        </div>
      </div>
              ...
              <div key={index} className="flex justify-between items-center text-sm">
                ...
                <div className="text-right">
                  ...
                  {metric.previousValue && (
                    <div className="text-xs text-gray-500">vs {metric.previousValue}</div>
                  )}
