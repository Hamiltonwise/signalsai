...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!googleClientId || !supabaseUrl) {
      const errorUrl = `${appOrigin}/oauth/callback?provider=gbp&status=error&error=missing_config`;
    ...
    console.log('GBP Start: Redirecting to Google OAuth');
    ...
    console.error('GBP Start error:', error);
    const appOrigin = Deno.env.get('APP_ORIGIN') || 'http://localhost:5173';
