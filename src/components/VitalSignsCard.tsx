...
export const VitalSignsCard: React.FC<VitalSignsCardProps> = ({
  ...
  const journeyStages: JourneyStage[] = [
    {
      ...
      score:
        aiAnalysis?.breakdown?.gscScore ||
        (connectionStatus.gsc ? gscData?.calculatedScore || 0 : 82),
      trend: connectionStatus.gsc
        ? parseFloat(gscData?.changePercent || "0")
        : 8,
      ...
      insight: connectionStatus.gsc
        ? `Your practice has ${(
          ...
          ).toLocaleString()} search impressions with ${(
            gscData?.totalClicks || 0
          ).toLocaleString()} clicks. Average position: ${(
            gscData?.averagePosition || 0
          ).toFixed(1)}.`
      ...
      action:
        aiAnalysis?.priorityOpportunities?.find(
          (op: any) => op.category === "search"
        )?.title ||
        (connectionStatus.gsc && (gscData?.averagePosition || 0) > 5
          ? "Improve keyword rankings - currently averaging position " +
            (gscData?.averagePosition || 0).toFixed(1)
          : 'Target "emergency dentist" keywords - 340% higher conversion potential'),
      metrics: [
        {
          label: "Search Impressions",
          value: connectionStatus.gsc
            ? (gscData?.totalImpressions || 0).toLocaleString()
            : "12.4K",
    ...
    },
    {
      ...
      score:
        aiAnalysis?.breakdown?.ga4Score ||
        (connectionStatus.ga4 ? ga4Data?.calculatedScore || 0 : 75),
      trend: connectionStatus.ga4
        ? parseFloat(ga4Data?.changePercent || "0")
        : -3,
      ...
      metrics: [
        {
          ...
          value: connectionStatus.ga4
            ? (ga4Data?.totalUsers || 0).toLocaleString()
            : "3.2K",
          status: connectionStatus.ga4
            ? (ga4Data?.totalUsers || 0) > 1000
              ? "good"
              : "warning"
            : "good",
        },
        ...
        },
        {
          label: "Conversions",
          value: connectionStatus.ga4
            ? (ga4Data?.conversions || 0).toString()
            : "126",
          status: connectionStatus.ga4
            ? (ga4Data?.conversions || 0) > 10
              ? "good"
              : "warning"
            : "good",
        },
    ...
    },
    {
      ...
      score:
        aiAnalysis?.breakdown?.gbpScore ||
        (connectionStatus.gbp ? gbpData?.calculatedScore || 0 : 91),
      trend: connectionStatus.gbp
        ? parseFloat(gbpData?.changePercent || "0")
        : 15,
      dataSource: "Google Business Profile",
      explainer:
        "The critical moment patients decide if your practice is right for them",
      whyItMatters:
        "Reviews and local reputation are the deciding factors. Patients compare you to competitors and seek social proof before making their choice.",
      insight: connectionStatus.gbp
        ? `${(gbpData?.totalViews || 0).toLocaleString()} profile views with ${(
            gbpData?.averageRating || 0
          ).toFixed(1)}★ rating from ${gbpData?.totalReviews || 0} reviews. ${
            gbpData?.phoneCallsTotal || 0
          } phone calls this period.`
        : 'Outstanding local reputation! 4.9★ rating drives 67% more bookings than competitors. Patients love your "gentle approach".',
      action:
        aiAnalysis?.priorityOpportunities?.find(
          (op: any) => op.category === "local"
        )?.title ||
        (connectionStatus.gbp && (gbpData?.averageRating || 0) < 4.5
          ? "Focus on improving review quality - currently at " +
      ...
      metrics: [
        ...
        },
        {
          label: "Phone Calls",
          value: connectionStatus.gbp
            ? gbpData?.phoneCallsTotal?.toString() || "0"
            : "8",
          status: connectionStatus.gbp
    ...
    },
    {
      ...
      score:
        aiAnalysis?.breakdown?.clarityScore ||
        (connectionStatus.clarity ? clarityData?.calculatedScore || 0 : 68),
      trend: connectionStatus.clarity
        ? parseFloat(clarityData?.changePercent || "0")
        : -8,
      dataSource: "Forms + Call Tracking + Clarity",
      explainer:
        "The moment of truth when patients take action to book an appointment",
      whyItMatters:
        "All your marketing efforts culminate here. A smooth booking process converts interested patients into scheduled appointments, while friction causes them to choose competitors.",
      insight: connectionStatus.clarity
        ? `${(
            clarityData?.totalSessions || 0
          ).toLocaleString()} sessions with ${(
            clarityData?.bounceRate || 0
          ).toFixed(1)}% bounce rate. ${
            clarityData?.deadClicks || 0
          } dead clicks detected.`
        : "Strong desktop conversions but mobile booking friction detected. 32% of users abandon mobile forms.",
      action:
        aiAnalysis?.priorityOpportunities?.find(
          (op: any) => op.category === "experience"
        )?.title ||
        (connectionStatus.clarity && (clarityData?.deadClicks || 0) > 10
          ? `Fix ${
              clarityData?.deadClicks || 0
            } dead clicks detected on your site`
          : "Streamline mobile booking - reduce from 8 fields to 4 essentials"),
      metrics: [
        {
          label: "Total Sessions",
          value: connectionStatus.clarity
            ? (clarityData?.totalSessions || 0).toLocaleString()
            : "15.4K",
          status: connectionStatus.clarity
            ? (clarityData?.totalSessions || 0) > 1000
              ? "good"
              : "warning"
            : "good",
        },
        {
          label: "Bounce Rate",
          value: connectionStatus.clarity
            ? `${(clarityData?.bounceRate || 0).toFixed(1)}%`
            : "32%",
          status: connectionStatus.clarity
            ? (clarityData?.bounceRate || 0) < 40
              ? "good"
              : "warning"
            : "good",
        },
        {
          label: "Dead Clicks",
          value: connectionStatus.clarity
            ? (clarityData?.deadClicks || 0).toString()
            : "45",
          status: connectionStatus.clarity
            ? (clarityData?.deadClicks || 0) < 20
              ? "good"
              : "critical"
            : "warning",
        },
    ...
    },
    {
      ...
      explainer:
        "Where one-time patients become lifelong advocates for your practice",
      whyItMatters:
        "Loyal patients are 5x more valuable than new ones. They return regularly, refer friends and family, and provide the foundation for sustainable practice growth.",
      insight: pmsData
        ? `${pmsData.totalPatients || 0} total referral events with ${
            pmsData.selfReferred || 0
          } self-referrals and ${pmsData.drReferred || 0} doctor referrals. $${(
            (pmsData.totalProduction || 0) / 1000
          ).toFixed(0)}K total production.`
        : "Excellent retention (78%) but untapped referral potential. Your loyal patients could generate 40% more revenue.",
      action:
        aiAnalysis?.priorityOpportunities?.find(
          (op: any) => op.category === "growth"
        )?.title ||
        (pmsData && pmsData.selfReferred < pmsData.drReferred
          ? "Boost self-referrals - currently only " +
            Math.round((pmsData.selfReferred / pmsData.totalPatients) * 100) +
            "% of referrals"
      ...
      metrics: [
        ...
        },
        {
          ...
          value: pmsData
            ? `${Math.round(
                (pmsData.selfReferred / Math.max(pmsData.totalPatients, 1)) *
                  100
              )}%`
            : "12%",
          status: pmsData
            ? pmsData.selfReferred / Math.max(pmsData.totalPatients, 1) > 0.3
              ? "good"
              : "warning"
            : "warning",
        },
        {
          label: "Production",
          value: pmsData
            ? `$${((pmsData.totalProduction || 0) / 1000).toFixed(0)}K`
            : "$94K",
          status: pmsData
            ? (pmsData.totalProduction || 0) > 50000
              ? "good"
              : "warning"
            : "good",
        },
    ...
    },
    {
      ...
      explainer:
        "Where it all comes together - tying our digital work to your business metrics",
      whyItMatters:
        "This is the ultimate measure of success. All digital marketing efforts should drive real business growth through increased referrals, higher production, and stronger referral partnerships.",
      insight: pmsData
        ? `${pmsData.totalPatients || 0} referrals this month generating $${(
            (pmsData.totalProduction || 0) / 1000
          ).toFixed(
            0
          )}K in production. Top sources: Website, Dr. Smith, Friend referrals.`
        : "Strong month with 156 referrals generating $94K in production. Top sources: Website (35%), Dr. Smith (28%), Friend referrals (22%).",
      action:
        aiAnalysis?.recommendations?.[0]?.title ||
        (pmsData && pmsData.selfReferred < pmsData.drReferred
          ? "Boost digital referrals - currently only " +
            Math.round(
              (pmsData.selfReferred / Math.max(pmsData.totalPatients, 1)) * 100
            ) +
            "% of referrals"
  ...
  const prevStage = () => {
    setActiveStageIndex(
      (prev) => (prev - 1 + journeyStages.length) % journeyStages.length
    );
  };
    ...
    <div className="bg-gradient-to-br from-white via-gray-50/30 to-blue-50/20 rounded-2xl shadow-xl border border-gray-200/50 backdrop-blur-sm">
      ...
      <div className="p-8 pb-6">
        <div className="flex items-center justify-between mb-8">
              ...
              <p className="text-gray-600 flex items-center space-x-2">
                ...
                <span>
                  {isLoadingAI
                    ...
                    : aiAnalysis
                    ? `AI insights • Grade ${overallGrade} • Updated ${
                        aiAnalysis.cached ? "from cache" : "now"
                      }`
          ...
          <div className="relative">
            <div
              className={`w-24 h-24 rounded-2xl ${getScoreGradient(
                overallScore
              )} p-0.5 shadow-2xl`}
            >
              ...
              <div className="flex items-center space-x-1">
                ...
                <span
                  className={`text-xs font-semibold ${
                    overallTrend > 0 ? "text-emerald-600" : "text-red-500"
                  }`}
                >
                  {overallTrend > 0 ? "+" : ""}
                  {overallTrend}%
      ...
      <div className="px-8 pb-8">
        <div className="relative">
          ...
          {/* Active Stage Card */}
          <div
            className={`${activeStage.bgGradient} rounded-2xl border-2 ${activeStage.borderGradient} p-8 shadow-lg transition-all duration-500 transform`}
          >
            {/* Stage Header */}
                ...
                <div className="relative">
                  ...
                  >
                    <div className="w-full h-full bg-white rounded-xl flex items-center justify-center">
                      <activeStage.icon
                        className={`w-8 h-8 ${activeStage.color}`}
                      />
                    </div>
                  </div>
              ...
              <div className="flex items-center space-x-2">
                ...
                <span
                  className={`text-lg font-semibold ${
                    activeStage.trend > 0 ? "text-emerald-600" : "text-red-500"
                  }`}
                >
                  {activeStage.trend > 0 ? "+" : ""}
                  {activeStage.trend}%
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-xl p-6 mb-6 border border-white/50 shadow-sm">
              <div className="flex items-start space-x-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-700 leading-relaxed">
                    {isLoadingAI
                      ? "AI is analyzing your practice data to provide personalized insights..."
                      : activeStage.insight}
                  </p>
                </div>
              </div>
            ...
            <div className="bg-gradient-to-r from-white to-gray-50/50 rounded-xl p-6 border border-gray-200/50 shadow-sm">
              <div className="flex items-center justify-between">
                <div className="flex items-start space-x-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-700">
                      {isLoadingAI
                        ? "AI is analyzing your data to provide the most impactful next step..."
                        : activeStage.action}
                    </p>
                    {aiAnalysis?.priorityOpportunities?.find(
                      (op: any) =>
                ...
                <button
                  className={`px-6 py-4 text-sm font-semibold text-white rounded-xl shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-200 ${
                    activeStage.color.includes("purple")
                      ? "bg-gradient-to-r from-purple-500 to-indigo-600"
                      : activeStage.color.includes("blue")
                      ? "bg-gradient-to-r from-blue-500 to-indigo-600"
                      : activeStage.color.includes("orange")
                      ? "bg-gradient-to-r from-orange-500 to-red-500"
                      : activeStage.color.includes("green")
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {(aiAnalysis.priorityOpportunities || [])
                      .slice(0, 2)
