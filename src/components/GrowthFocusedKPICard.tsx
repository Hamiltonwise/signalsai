...
import { TrendingUp, TrendingDown, Activity, DivideIcon as LucideIcon } from 'lucide-react';
...
export const GrowthFocusedKPICard: React.FC<GrowthFocusedKPICardProps> = ({
  ...
  const getStatusBadge = () => {
    if (trend === 'up' && changePercent > 10) {
      return { text: '🚀 Improving Fast', color: 'bg-green-100 text-green-800' };
  ...
  const sparklineData = last12MonthsData || Array.from({ length: 12 }, (_, i) => {
    const base = 50;
    const variation = isConnected ? 
      (trend === 'up' ? i * 2 : trend === 'down' ? -i * 1.5 : Math.sin(i) * 5) :
      (trend === 'up' ? i * 3 : trend === 'down' ? -i * 2 : Math.sin(i) * 8);
  ...
  return (
    <div className="bg-white rounded-xl shadow-md border border-gray-200 p-6 hover:shadow-lg transition-all duration-300 group">
      ...
      <div className="mb-4">
          ...
          {sparklineData.map((point, index) => (
            <div
              ...
              className={`flex-1 rounded-t transition-all duration-500 ${
                trend === 'up' ? 'bg-gradient-to-t from-green-200 to-green-400' :
                trend === 'down' ? 'bg-gradient-to-t from-amber-200 to-amber-400' :
        ...
        <div className="text-xs text-gray-500 mt-1 text-center">12-month trend</div>
      </div>
